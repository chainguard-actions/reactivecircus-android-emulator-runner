...
var require_sdk_installer = __commonJS({
  "lib/sdk-installer.js"(exports2) {
    "use strict";
    var __createBinding2 = exports2 && exports2.__createBinding || (Object.create ? (function(o, m, k, k2) {
      if (k2 === void 0) k2 = k;
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.installAndroidSdk = installAndroidSdk;
    var core2 = __importStar2(require_core());
    var exec2 = __importStar2(require_exec());
    var io = __importStar2(require_io());
    var tc = __importStar2(require_tool_cache());
    var fs = __importStar2(require("fs"));
    var BUILD_TOOLS_VERSION = "36.0.0";
    var CMDLINE_TOOLS_URL_MAC = "https://dl.google.com/android/repository/commandlinetools-mac-14742923_latest.zip";
    var CMDLINE_TOOLS_URL_LINUX = "https://dl.google.com/android/repository/commandlinetools-linux-14742923_latest.zip";
    async function installAndroidSdk(apiLevel, systemImageApiLevel, target, arch, channelId, emulatorBuild, ndkVersion, cmakeVersion) {
      try {
        ...
        core2.addPath(`${cmdlineToolsPath}/latest:${cmdlineToolsPath}/latest/bin:${process.env.ANDROID_HOME}/platform-tools`);
        await io.mkdirP(`${process.env.HOME}/.android/avd`);
        core2.exportVariable("ANDROID_AVD_HOME", `${process.env.HOME}/.android/avd`);
        await exec2.exec(`sh -c \\"yes | sdkmanager --licenses > /dev/null"`);
        console.log("Installing latest build tools, platform tools, and platform.");
        await exec2.exec(`sh -c \\"sdkmanager --install 'build-tools;${BUILD_TOOLS_VERSION}' platform-tools 'platforms;android-${apiLevel}'> /dev/null"`);
        console.log("Installing latest emulator.");
        await exec2.exec(`sh -c \\"sdkmanager --install emulator --channel=${channelId} > /dev/null"`);
        if (emulatorBuild) {
          console.log(`Installing emulator build ${emulatorBuild}.`);
          var downloadUrlSuffix;
          const majorBuildVersion = Number(emulatorBuild);
          if (majorBuildVersion >= 8e6) {
            if (isArm) {
              downloadUrlSuffix = `_aarch64-${emulatorBuild}`;
            } else {
              downloadUrlSuffix = `_x64-${emulatorBuild}`;
            }
          } else if (majorBuildVersion >= 7e6) {
            downloadUrlSuffix = `_x64-${emulatorBuild}`;
          } else {
            downloadUrlSuffix = `-${emulatorBuild}`;
          }
          await exec2.exec(`curl -fo emulator.zip https://dl.google.com/android/repository/emulator-${isOnMac ? "darwin" : "linux"}${downloadUrlSuffix}.zip`);
          await exec2.exec(`unzip -o -q emulator.zip -d ${process.env.ANDROID_HOME}`);
          await io.rmRF("emulator.zip");
        }
        console.log("Installing system images.");
        await exec2.exec(`sh -c \\"sdkmanager --install 'system-images;android-${systemImageApiLevel};${target};${arch}' --channel=${channelId} > /dev/null"`);
        if (ndkVersion) {
          console.log(`Installing NDK ${ndkVersion}.`);
          await exec2.exec(`sh -c \\"sdkmanager --install 'ndk;${ndkVersion}' --channel=${channelId} > /dev/null"`);
        }
        if (cmakeVersion) {
          console.log(`Installing CMake ${cmakeVersion}.`);
          await exec2.exec(`sh -c \\"sdkmanager --install 'cmake;${cmakeVersion}' --channel=${channelId} > /dev/null"`);
        }
      } finally {
...
var require_input_validator = __commonJS({
  "lib/input-validator.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.MAX_PORT = exports2.MIN_PORT = exports2.VALID_CHANNELS = exports2.VALID_ARCHS = exports2.MIN_API_LEVEL = void 0;
    exports2.playstoreTargetSubstitution = playstoreTargetSubstitution;
    exports2.checkArch = checkArch;
    exports2.checkChannel = checkChannel;
    exports2.checkForceAvdCreation = checkForceAvdCreation;
    exports2.checkPort = checkPort;
    exports2.checkDisableAnimations = checkDisableAnimations;
    exports2.checkDisableSpellchecker = checkDisableSpellchecker;
    exports2.checkDisableLinuxHardwareAcceleration = checkDisableLinuxHardwareAcceleration;
    exports2.checkEnableHardwareKeyboard = checkEnableHardwareKeyboard;
    exports2.checkEmulatorBuild = checkEmulatorBuild;
    exports2.checkDiskSize = checkDiskSize;
    exports2.MIN_API_LEVEL = 15;
    exports2.VALID_ARCHS = ["x86", "x86_64", "arm64-v8a"];
    exports2.VALID_CHANNELS = ["stable", "beta", "dev", "canary"];
    exports2.MIN_PORT = 5554;
    exports2.MAX_PORT = 5584;
    ...
    function checkArch(arch) {
      if (!exports2.VALID_ARCHS.includes(arch)) {
        throw new Error(`Value for input.arch '${arch}' is unknown. Supported options: ${exports2.VALID_ARCHS}.`);
      }
    }
    function checkChannel(channel) {
      if (!exports2.VALID_CHANNELS.includes(channel)) {
        throw new Error(`Value for input.channel '${channel}' is unknown. Supported options: ${exports2.VALID_CHANNELS}.`);
      }
    }
    ...
    function checkPort(port) {
      if (port < exports2.MIN_PORT || port > exports2.MAX_PORT) {
        throw new Error(`Emulator port is outside of the supported port range [${exports2.MIN_PORT}, ${exports2.MAX_PORT}], was ${port}`);
      }
      if (port % 2 == 1) {
...
var require_emulator_manager = __commonJS({
  "lib/emulator-manager.js"(exports2) {
    "use strict";
    var __createBinding2 = exports2 && exports2.__createBinding || (Object.create ? (function(o, m, k, k2) {
      if (k2 === void 0) k2 = k;
      var desc = Object.getOwnPropertyDescriptor(m, k);
    ...
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.createAvd = createAvd;
    exports2.launchEmulator = launchEmulator;
    exports2.killEmulator = killEmulator;
    var exec2 = __importStar2(require_exec());
    var fs = __importStar2(require("fs"));
    async function createAvd(arch, avdName, cores, diskSize, enableHardwareKeyboard, forceAvdCreation, heapSize, profile, ramSize, sdcardPathOrSize, systemImageApiLevel, target) {
      try {
        ...
        if (!fs.existsSync(avdPath) || forceAvdCreation) {
          ...
          await exec2.exec(`sh -c \\"echo no | avdmanager create avd --force -n "${avdName}" --package 'system-images;android-${systemImageApiLevel};${target};${arch}' ${profileOption} ${sdcardPathOrSizeOption}"`);
        }
        if (cores || ramSize || heapSize || enableHardwareKeyboard || diskSize) {
          ...
          if (configEntries.length > 0) {
            ...
            await exec2.exec(`sh -c \\"printf '${configContent}' >> ${process.env.ANDROID_AVD_HOME}/"${avdName}".avd"/config.ini"`);
          }
        }
      } finally {
        console.log(`::endgroup::`);
    ...
    async function launchEmulator(avdName, disableAnimations, disableLinuxHardwareAcceleration, disableSpellChecker, emulatorBootTimeout, emulatorOptions, enableHardwareKeyboard, port) {
      try {
        ...
        await exec2.exec(`sh -c \\"${process.env.ANDROID_HOME}/emulator/emulator -port ${port} -avd "${avdName}" ${emulatorOptions} &"`, [], {
          listeners: {
            stderr: (data) => {
    ...
    async function adb(port, command) {
      return await exec2.exec(`adb -s emulator-${port} ${command}`);
    }
    async function waitForDevice(port, emulatorBootTimeout) {
      ...
      while (!booted) {
        try {
          let result = "";
          await exec2.exec(`adb -s emulator-${port} shell getprop sys.boot_completed`, [], {
            listeners: {
              stdout: (data) => {
...
var require_script_parser = __commonJS({
  "lib/script-parser.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.parseScript = parseScript;
    function parseScript(rawScript) {
      const scripts = rawScript.trim().split(/\r\n|\n|\r/).map((value) => value.trim()).filter((value) => {
...
var require_channel_id_mapper = __commonJS({
  "lib/channel-id-mapper.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.getChannelId = getChannelId;
...
var sdk_installer_1 = require_sdk_installer();
var input_validator_1 = require_input_validator();
var emulator_manager_1 = require_emulator_manager();
var exec = __importStar(require_exec());
var script_parser_1 = require_script_parser();
var channel_id_mapper_1 = require_channel_id_mapper();
var fs_1 = require("fs");
async function run() {
  ...
  try {
    ...
    const preEmulatorLaunchScripts = !preEmulatorLaunchScriptInput ? void 0 : (0, script_parser_1.parseScript)(preEmulatorLaunchScriptInput);
    ...
    if (preEmulatorLaunchScripts !== void 0) {
